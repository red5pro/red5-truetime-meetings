...
const validateConfig = (config) => {
    ...
    if (config.VITE_ENABLE_RECORDING !== undefined && !validBooleans.includes(config.VITE_ENABLE_RECORDING)) {
        errors.push('VITE_ENABLE_RECORDING must be "true" or "false"');
    }

    if (config.VITE_ENABLE_CLOSED_CAPTION !== undefined && !validBooleans.includes(config.VITE_ENABLE_CLOSED_CAPTION)) {
