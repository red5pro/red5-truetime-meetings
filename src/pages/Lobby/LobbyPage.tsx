...
const LobbyPage = React.memo<LobbyPageProps>((props) => {
    ...
    const handleJoinRoom = useCallback((e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
    ...
    const handleStreamNameChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value
        setStreamNameFieldError(value.trim().length === 0)
        setStreamName(value)
    }, [setStreamName])

    const handleRoleChange = useCallback((e: SelectChangeEvent<UserRole>) => {
        setRole(e.target.value as UserRole)
    }, [setRole])

    const handleSettingsOpen = useCallback((focus: string | null = null) => {
        if (devices.length === 0) {
            showDeviceError()
            return
        }
