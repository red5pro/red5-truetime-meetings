...
function Red5(props: Red5Props) {
    ...
    const makeFullScreen = useCallback((divId: string) => {
        ...
        if (sharedVariables.fullScreenId === divId) {
            ...
            const publisherContent = document.getElementsByClassName('publisher-content')[0] as HTMLElement;
            if (publisherContent) {
    ...
    const turnOffYourMicNotification = useCallback((participantId: string) => {
        conferenceRef.current.room.sendNotificationEvent("TURN_YOUR_MIC_OFF", conferenceRef.current.room.publishStreamIdRef.current, {
            ...
            >
                ...
                {conference.room.isJoining && (
                    ...
                    >
                        <Stack alignItems="center" justifyContent="center" alignContent="center">
                            ...
                            ) : Object.keys(conference.participants.participants).length === 0 && conference.room.isPlayOnly ? (
                                <span style={{ margin: '27px', fontSize: 18, fontWeight: 'normal' }}>
                                    <b>{t('The room is currently empty.')}</b><br />
                ...
                {conference.room.leftTheRoom ? (
                    ...
                    <LobbyPage
                        ...
                        handleMuteVideoLobby={() => conference.media.toggleCamera(!conference.media.isMyCamTurnedOff)}
                        ...
                        setSelectedBackgroundMode={conference.features.virtualBackground.setSelectedBackgroundMode}
                        handleBackgroundReplacement={conference.features.virtualBackground.handleBackgroundReplacement}
                        setVirtualBackgroundImage={conference.features.virtualBackground.setVirtualBackgroundImage}
                    ...
                    <>
                        <MeetingPage
                            ...
                            startRecord={(recordSeparately?: boolean, serverRecording?: boolean, localRecording?: boolean) => conference.features.recording.startRecording(recordSeparately, serverRecording, localRecording)}
                            stopRecord={(serverRecording?: boolean, localRecording?: boolean) => conference.features.recording.stopRecording(serverRecording, localRecording)}
                        ...
                        <MessageDrawer
                            ...
                            handleParticipantListOpen={conference.ui.handleParticipantListOpen}
                            handleEffectsOpen={conference.ui.handleEffectsOpen}
                            handleLocalRecordingDrawerOpen={(open: boolean) => conference.ui.handleLocalRecordingDrawerOpen?.(open)}
                            handleTranscriptionDrawerOpen={(open: boolean) => conference.ui.handleTranscriptionDrawerOpen?.(open)}
                        />
