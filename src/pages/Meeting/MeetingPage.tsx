...
const MeetingPage = React.memo<MeetingPageProps>((props) => {
  ...
  const handleGalleryResize = useCallback((calcDrawer = false) => {
    ...
    if (calcDrawer) {
      const currentProps = propsRef.current;
      const hasOpenDrawer =
        currentProps.messageDrawerOpen ||
    ...
    <Container id="meeting-page">
      ...
      <ClosedCaptions
        ...
        onLanguageChange={(language) =>
          props?.closedCaptions.handleCaptionsLanguageChange(language)
        }
      ...
      <Footer
        ...
        startRecord={(recordSeparately, serverRecording, localRecording) =>
          props?.startRecord?.(recordSeparately, serverRecording, localRecording)
        }
        stopRecord={(serverRecording, localRecording) =>
        ...
        onCaptionsLanguageChange={(language: any) =>
          props?.closedCaptions.handleCaptionsLanguageChange(language)
        }
        isLiveCaptions={props?.closedCaptions.isLiveCaptions}
