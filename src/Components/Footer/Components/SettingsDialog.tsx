...
export default function SettingsDialog(props: SettingsDialogProps) {
    ...
    const [localPushToTalkEnabled, setLocalPushToTalkEnabled] = React.useState<boolean>(props?.pushToTalkEnabled || false);
    ...
    React.useEffect(() => {
        if (props?.devices) {
            ...
            if (cameraDevices && (cameraDevices.length > 0) && (props?.selectedCamera === '' || isNull(props?.selectedCamera))) {
                props?.cameraSelected?.(cameraDevices[0].deviceId);
            }
            if (audioDevices && (audioDevices.length > 0) && (props?.selectedMicrophone === '' || isNull(props?.selectedMicrophone))) {
                props?.microphoneSelected?.(audioDevices[0].deviceId);
            }
            if (speakerDevices && (speakerDevices.length > 0) && (props?.selectedSpeaker === '' || isNull(props?.selectedSpeaker))) {
                ...
                <Box component="form" sx={{ display: 'flex', flexWrap: 'wrap' }}>
                    ...
                    <Grid container>
                        ...
                        <Grid size={12} sx={{width: '100vw'}}>
                            ...
                            >
                                {props?.devices?.videoInputs && props?.devices?.videoInputs?.length > 0 && props?.devices?.videoInputs
                                    .map(device => (
                    ...
                    <Grid container sx={{ mt: 4 }}>
                        <Grid size={12}>
                            <InputLabel sx={{ color: '#fff', mb: 1 }}>{t('Outgoing Video Quality Limit')}</InputLabel>
                        </Grid>
                        <Grid size={12} sx={{width: '100vw'}}>
                            <Select
                                variant="outlined"
                                ...
                                endAdornment={
                                    <InputAdornment position="end" sx={{ mr: 2 }}>
                                        <SvgIcon viewBox={'0 0 24 24'} size={24} name={'bitrate-limit'} color={'#fff'} />
                                    </InputAdornment>
                                }
                            >
                    ...
                    <Grid container sx={{ mt: 4 }}>
                        ...
                        <Grid size={12} sx={{width: '100vw'}}>
                            ...
                            >
                                {props?.devices?.audioOutputs && props?.devices?.audioOutputs?.length > 0 && props?.devices?.audioOutputs
                                    .map(device => (
                                        <MenuItem key={device.deviceId} value={device.deviceId}>
                                            {device.label}
                                        </MenuItem>
                    ...
                    <Grid container sx={{ mt: 4 }}>
                        ...
                        <Grid size={12} sx={{width: '100vw'}}>
                            ...
                            >
                                {props?.devices?.audioInputs && props?.devices?.audioInputs?.length > 0 && props?.devices?.audioInputs
                                    .map(device => (
                                        <MenuItem key={device.deviceId} value={device.deviceId}>
                                            {device.label}
                                        </MenuItem>
