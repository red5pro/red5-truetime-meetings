...
import { ListItemIcon, ListItemText, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, Button, FormControlLabel, Checkbox, Box } from '@mui/material'
...
function OptionButton(props: OptionButtonProps) {
    ...
    const handlePiPClick = (): void => {
        if (!props?.pipSupported) {
            console.warn('Picture-in-Picture is not supported in this browser. Please use Chrome 111+ or Edge 111+')
            return
        }
    ...
    const handleFullScreenToggle = (): void => {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().then(r => log.log('Fullscreen is requested', r))
        } else {
    ...
    return (
        <>
            ...
            >
                ...
                <DialogActions>
                    ...
                    <Button onClick={handleRecordingConfirm} variant="contained">{t('Start Recording')}</Button>
                </DialogActions>
            </Dialog>
        ...
        */}
                ...
                {props?.isPlayOnly === false && (
                    <MenuItem key="call-settings" onClick={() => {
                ...
                {((props?.isRecordingActive === false) && (props?.isLocalRecordingActive === false)) && (
                    ...
                    >
                        ...
                        <ListItemText
                            primary={!isRecordingEnabled ? t('Upgrade your plan to support recording') : t('Start Record')}
                            sx={{
                                '& .MuiListItemText-primary': {
                ...
                {((props?.isRecordingActive === true) || (props?.isLocalRecordingActive === true)) && (
                    ...
                    >
                        ...
                        <ListItemText
                            primary={!isRecordingEnabled ? t('Upgrade your plan to support recording') : t('Stop Record')}
                            sx={{
                                '& .MuiListItemText-primary': {
                                    color: !isRecordingEnabled ? 'text.disabled' : 'text.primary'
                                }
                ...
                <MenuItem key="full-screen" onClick={handleFullScreenToggle} id="full-screen">
                    ...
                    <ListItemText>{document.fullscreenElement ? t('Exit full screen') : t('Full screen')}</ListItemText>
                </MenuItem>

                {props?.isPlayOnly === false && (
                    <MenuItem key="virtual-effects" onClick={handleVirtualEffects} id="virtual-effects">
                        <ListItemIcon>
                            <SvgIcon size={24} viewBox="0 0 500 500" name={"virtual-effects"}
                                color={getMenuIconColor()} />
                        </ListItemIcon>
