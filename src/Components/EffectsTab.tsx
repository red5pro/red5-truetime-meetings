...
const SectionHeader = React.memo<SectionHeaderProps>(({ title, icon, theme }) => (
    <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
        {icon && <SvgIcon size={20} viewBox="0 0 500 500" name={icon} color={theme.palette.text.primary} />}
        <Typography
            variant="subtitle1"
            sx={{
                ml: icon ? 1 : 0,
...
const VirtualBackgroundButton = React.memo<VirtualBackgroundButtonProps>(({
    ...
    React.useEffect(() => {
        ...
        if (imageLoadPromises.has(imageSrc)) {
            imageLoadPromises.get(imageSrc)
...
function EffectsTab({ setVirtualBackgroundImage, handleBackgroundReplacement }: EffectsTabProps): JSX.Element {
    ...
    const virtualBackgroundImageData = React.useMemo(() => {
        if (getRuntimeConfig().VITE_VIRTUAL_BACKGROUND_IMAGES !== undefined &&
    ...
    const saveImageToFileSystem = React.useCallback(async (selectedFile: File, directoryHandle: any): Promise<void> => {
        log.log("Saving image to file system", selectedFile, directoryHandle);
    }, []);

    // Stable callbacks using useCallback instead of refs
    const handleEffectSelection = React.useCallback((effectType: string) => {
        setSelectedEffect(effectType);
        setSelectedBackground(null);
        handleBackgroundReplacement({ type: effectType });
    }, [handleBackgroundReplacement]);

    const handleBackgroundSelection = React.useCallback((imageSrc: string, type: string = 'image') => {
