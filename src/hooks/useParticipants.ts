...
export const useParticipants = (): UseParticipantsReturn => {
    ...
    const updateTalkers = useCallback((): void => {
        const updatedTalkers = Object.keys(talkerAudioLevelsRef.current)
            .filter(streamId => talkerAudioLevelsRef.current[streamId] > 75);
        setTalkers(updatedTalkers);
    }, []);

    const updateTalkerLevel = useCallback((userId: string, level: number): void => {
