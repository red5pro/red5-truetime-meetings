...
export const useChat = (
        ...
        try {
            ...
            if (files && files.length > 0) {
                ...
                if (fileResults && fileResults.length > 0) {
                    ...
                    conferenceClientRef.current?.sendMessageWithFiles(
                        message,
                        streamName,
                        fileResults
                    );
    ...
    const handleChatMessage = useCallback((chatMessage: ChatMessage) => {
        ...
        let filesArray: FileType[] | undefined = chatMessage.messageEvent.message.files
        if (filesArray) {
            for (let i = 0; i < filesArray.length; ++i) {
                filesArray[i].url = conferenceClientRef.current?.getFileUrl(filesArray[i].id, filesArray[i].name);
        ...
        if (eventType === 'MESSAGE_RECEIVED') {
            ...
            if (!messageDrawerOpen) {
                let notificationMessage = notificationEvent.message ? truncateText(notificationEvent.message) : '';
            ...
            if (notificationEvent.isRaisedHand) {
                ...
                setRaisedHandsRef.current(prev =>
                    // @ts-ignore
                    prev.filter(streamId => streamId.localeCompare(notificationEvent.senderStreamId) !== 0)
                );
            }
