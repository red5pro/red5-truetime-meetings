...
export const useDeviceManagement = (
    ...
    const getSelectedDevices = useCallback((): SelectedDevices => ({
    ...
    const cameraSelected = useCallback((value: string): void => {
    ...
    const microphoneSelected = useCallback((value: string): void => {
    ...
    const speakerSelected = useCallback(async (value: string): Promise<void> => {
        let speaker = value;
        if (speaker) {
            ...
            for (const element of audioElements) {
                if ('setSinkId' in element && typeof element.setSinkId === 'function') {
                    try {
                        ...
                        log.info(`Successfully switched speaker output for element ${element.id || 'unnamed'} to device: ${speaker}`);
                    } catch (sinkError) {
                        log.warn(`Failed to set speaker output for element ${element.id || 'unnamed'}:`, sinkError);
    ...
    const checkDeviceAvailability = (selectedDevices: SelectedDevices): DeviceAvailability => {
        const videoDevice = devicesRef.current.videoInputs?.find(d => d.deviceId === selectedDevices.videoDeviceId);
        const audioDevice = devicesRef.current.audioInputs?.find(d => d.deviceId === selectedDevices.audioDeviceId);
    ...
    const updateMediaControlsBasedOnAvailability = ({ isVideoAvailable, isAudioAvailable }: DeviceAvailability): void => {
        if (isVideoAvailable) {
    ...
    const handleUnavailableDevices = (selectedDevices: SelectedDevices, { isVideoAvailable, isAudioAvailable }: DeviceAvailability): SelectedDevices => {
    ...
    const handleUnavailableVideoDevice = (): string => {
        ...
        if (availableCamera) {
            ...
            displayMessageRef.current('Unable to access selected camera, switching to first available camera.');
            return availableCamera.deviceId;
        } else {
            // No camera available - disable video
            clientRef.current.muteVideo(true).then(() => mediaControlsRef.current.setIsMyCamTurnedOff(true));
    ...
    const handleUnavailableAudioDevice = (): string => {
        ...
        if (availableAudio) {
            ...
            displayMessageRef.current('Unable to access selected microphone, switching to first available microphone.');
    ...
    const hasDevicesChanged = (original: SelectedDevices, updated: SelectedDevices): boolean => {
        return original.videoDeviceId !== updated.videoDeviceId ||
            original.audioDeviceId !== updated.audioDeviceId;
    };

    const switchMediaSourcesIfNeeded = (previousVideoId: string | null | undefined, previousAudioId: string | null | undefined, currentDevices: SelectedDevices): void => {
        ...
        try {
            ...
            const shouldSwitchAudio = previousAudioId !== currentDevices.audioDeviceId ||
