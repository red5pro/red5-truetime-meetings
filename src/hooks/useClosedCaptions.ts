...
export const useClosedCaptions = (client: Client): UseClosedCaptionsReturn => {
    ...
    const addCaption = useCallback((captionData: CaptionData) => {
        ...
        setCaptions(prev => {
            ...
            const isDuplicate = updated.some(caption =>
    ...
    const getCaptionsForSpeaker = useCallback((speakerId: string): Caption[] => {
        return captions.filter(caption => caption.speaker === speakerId || caption.streamId === speakerId);
    }, [captions]);
