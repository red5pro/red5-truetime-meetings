...
export const useConferenceClient = () => {
            ...
            if (conferenceClient.current) {
                ...
                if (conferenceClient.current.getIsJoined()) {
                    conferenceClient.current
                        .leave()
                        .catch((err: any) =>
                            log.error("Error leaving room on cleanup:", err)
                        );
                }
    ...
    const switchVideoDevice = useCallback(async (deviceId: string) => {
        ...
        try {
            const mediaStream = await conferenceClient.current.switchVideoDeviceWithTrackReplacement(
                deviceId
            );
    ...
    const switchAudioDevice = useCallback(async (deviceId: string) => {
        ...
        try {
            const mediaStream = await conferenceClient.current.switchAudioDeviceWithTrackReplacement(
                deviceId
            );
     ...
     */
    const switchVideoCameraCapture = useCallback(
        async (streamId: string, deviceId: string) => {
            if (!conferenceClient.current) return false;

            try {
                await conferenceClient.current.switchVideoDeviceWithTrackReplacement(
                    deviceId
                );
     ...
     */
    const switchAudioInputSource = useCallback(
        async (streamId: string, deviceId: string) => {
            if (!conferenceClient.current) return false;

            try {
                await conferenceClient.current.switchAudioDeviceWithTrackReplacement(
                    deviceId
                );
     ...
     */
    const changeVirtualBackground = useCallback(
        async (type: string, options: any) => {
            if (!conferenceClient.current) return false;
     ...
     */
    const getVirtualBackgroundStatus = useCallback(
    ...
    const getCurrentStream = useCallback(() => {
        if (
            !conferenceClient.current ||
            !conferenceClient.current.mediaStreamManager
        )
            return null;
