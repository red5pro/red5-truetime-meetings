...
type ParticipantsHook = {
    ...
    subscribeAttemptsRef: React.MutableRefObject<Record<string, { retryCount: number; inProgress: boolean }>>;
...
export const useConferenceEvents = (
    ...
    if (!eventHandlersRef.current.initialized) {
        eventHandlersRef.current = {
            ...
            handleTranscriptionResult: (data: any) => {
                ...
                let captionData = data.messageEvent;
            ...
            handleConnectionClosed: () => {
                ...
                depsRef.current.handleLeaveFromRoom().then(() =>
                    depsRef.current.roomState.setLobbyOrMeetingPage('lobby')
                );
            },
            ...
            handleSubscribeFailed: (data: any) => {
                ...
                // @ts-ignore
                log.warn(`Subscription failed for ${data.user.uid}. Attempt ${participantsHook.subscribeAttemptsRef.current[data.user.uid].retryCount}/${globals.maxRetries}. Will retry on next opportunity.`);
            ...
            handleJoinFail: async (data: any) => {
                ...
                depsRef.current.handleLeaveFromRoom().then(() => console.log('handleLeaveFromRoom due to join fail'));

                if (data.statusCode === 401) {
                    depsRef.current.roomState.setIsJoining(false);
                    depsRef.current.roomState.setIsWaitingApproval(false);
                    depsRef.current.setUnAuthorizedDialogMessage('Publish failed, due to an error. Please try again.');
            ...
            handleJoinBlock: async (data: any) => {
                ...
                let errorMessage = 'You are blocked in this room. Please contact the room owner to unblock you.';
                if (depsRef.current.role === 'guest') {
            ...
            handleRoomStateUpdate: (data: any) => {
                ...
                } else if (data.roomState.localRecordingEnabled === false && depsRef.current.recording.isLocalRecordingActive) {
            ...
            handleGuestJoinRequest: (data: any) => {
                ...
                const participantsHook = depsRef.current.participantsHook;
                let guestsWaitingApproval = { ...(participantsHook.guestsWaitingApproval || {}) };
            ...
            handleNewParticipant: async (data: any) => {
                ...
                let newParticipant = data.participant;
                let newParticipantMetaData = parseMetaData(data.participant.metaData);
                ...
                newParticipant.isScreenSharing = newParticipantMetaData[MetaDataKeys.IS_SCREEN_SHARING] || false;
            ...
            handleUserPublished: async (data: any) => {
                ...
                for (const [userId, _participant] of Object.entries(participants)) {
                    let updatedParticipant = participants[userId];
                    let participantMetaData = parseMetaData(updatedParticipant.metaData);
                    updatedParticipant.name = participantMetaData[MetaDataKeys.NAME] || updatedParticipant.uid;
                    updatedParticipant.isRaiseHand = participantMetaData[MetaDataKeys.IS_RAISED_HAND] || false;
            ...
            updateDetailedStats: () => {
                if (client.conferenceClient.current && client.conferenceClient.current.isJoined) {
                    ...
                    if (client.conferenceClient.current.streamName) {
                        const publisherStats = client.conferenceClient.current.getConnectionStats(client.conferenceClient.current.streamName);
                    ...
                    if (client.conferenceClient.current.isScreenSharing && client.conferenceClient.current.streamName) {
            ...
            subscribeToParticipant: async (participant: any) => {
                ...
                try {
                    ...
                    // @ts-ignore
                    if (participantsHook.subscribeAttemptsRef.current[participant.uid]?.retryCount >= globals.maxRetries) {
                        log.error(`Max subscription attempts reached for ${participant.uid}. Removing participant.`);
                        ...
                        log.log(`Removed participant ${participant.uid} from DOM due to repeated subscription failure`);
                        return;
                    }
            ...
            clearRemoteSubscriber: (streamId: any) => {
                ...
                if (participantsHook.pinnedParticipantIdRef.current && streamId.localeCompare(participantsHook.pinnedParticipantIdRef.current) === 0) {
    ...
    useEffect(() => {
        ...
        const events: Record<string, (...args: any[]) => void> = {
            ...
            [ConferenceEvents.PARTICIPANT_DISCONNECTED]: eventHandlersRef.current.handleParticipantDisconnected,
            ...
            [ConferenceEvents.PARTICIPANT_MEDIA_UPDATE]: eventHandlersRef.current.handleParticipantMediaUpdate,
            ...
            [ConferenceEvents.NETWORK_SCORES_UPDATED]: eventHandlersRef.current.handleNetworkScoresUpdated,
            [ConferenceEvents.WEBRTC_ISSUES_DETECTED]: eventHandlersRef.current.webrtcIssuesDetected,
            [ConferenceEvents.CONNECTION_STATE_CHANGED]: eventHandlersRef.current.handleConnectionStateChanged,
            [ConferenceEvents.CONNECTION_FAILED]: eventHandlersRef.current.handleConnectionFailed,
            [ConferenceEvents.VIRTUAL_BACKGROUND_INITIALIZED]: eventHandlersRef.current.handleEnableVirtualBackgroundControls,
            [ConferenceEvents.VIRTUAL_BACKGROUND_ENABLED]: eventHandlersRef.current.handleVirtualBackgroundEnabled,
            [ConferenceEvents.VIRTUAL_BACKGROUND_DISABLED]: eventHandlersRef.current.handleVirtualBackgroundDisabled,
            [ConferenceEvents.VIRTUAL_BACKGROUND_CHANGED]: eventHandlersRef.current.handleVirtualBackgroundChanged,
            [ConferenceEvents.VIRTUAL_BACKGROUND_ENABLE_FAILED]: eventHandlersRef.current.handleVirtualBackgroundEnableFailed,
            [ConferenceEvents.TRANSCRIPTION_RESULT]: eventHandlersRef.current.handleTranscriptionResult,
            [ConferenceEvents.LOCAL_RECORDING_ENABLED]: eventHandlersRef.current.handleLocalRecordingEnabled,
        };
