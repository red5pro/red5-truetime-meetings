 ...
 * ```
 */
export function useMediabunnyRecording(
  options: UseMediabunnyRecordingOptions = {},
): UseMediabunnyRecordingReturn {
  const { onRecordingStart, onRecordingStop, onError, onPause, onResume, recordingOptions } =
    options;
  ...
  const downloadRecording = useCallback(
    async (baseFilename?: string): Promise<boolean> => {
      ...
      try {
        ...
        blobs.forEach((blob, index) => {
          const filename =
            blobs.length === 1 ? `recording.${extension}` : `part${index + 1}.${extension}`;
        ...
        const timestamp = (startTimeRef.current ? new Date(startTimeRef.current) : new Date())
          .toISOString()
          .replace(/[:.]/g, '-');
