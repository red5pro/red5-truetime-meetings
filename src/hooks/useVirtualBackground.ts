...
export const useVirtualBackground = (
   ...
   */
  const handleBackgroundReplacement = useCallback(
    async (option: VirtualBackgroundOption): Promise<boolean> => {
      if (!conferenceClientRef?.current) {
        log.error('Conference client not initialized');
        return false;
      }
      ...
      try {
        ...
        const shouldEnable =
          effectStatus.backgroundType === VirtualBackgroundTypes.NONE && !effectStatus.isEnabled;
   ...
   */
  const setVirtualBackgroundImage = useCallback(
    async (options?: VirtualBackgroundOptions): Promise<boolean> => {
  ...
  const applyBlurBackground = useCallback(
    async (blurAmount: number = 50): Promise<boolean> => {
   ...
   */
  const applyColorBackground = useCallback(
