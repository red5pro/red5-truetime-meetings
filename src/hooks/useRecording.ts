...
export const useRecording = (
  ...
  const hasS3Config = !!(
    config.VITE_AWS_ACCESS_KEY &&
    config.VITE_AWS_SECRET_ACCESS_KEY &&
    config.VITE_AWS_BUCKET_NAME
  );
  ...
  const [localRecordingStatus, setLocalRecordingStatus] = useState<LocalRecordingStatus | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_STARTED,
      handleRecordingStarted as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_STOPPED,
      handleRecordingStopped as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_PAUSED,
      handleRecordingPaused as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_RESUMED,
      handleRecordingResumed as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_ERROR,
      handleRecordingError as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_DOWNLOADED,
      handleRecordingDownloaded as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_DATA,
      handleRecordingData as EventListener,
    );
    client.addEventListener(
      ConferenceEvents.LOCAL_RECORDING_CLEARED,
  ...
  const startRecording = useCallback(
    async (
      recordSeparately: boolean = false,
      serverRecording: boolean = true,
      localRecording: boolean = true,
    ): Promise<boolean> => {
      if (!roomName) {
  ...
  const stopRecording = useCallback(
    async (serverRecording: boolean = true, localRecording: boolean = true): Promise<boolean> => {
      if (!roomName) {
  ...
  const startLocalRecording = useCallback(
    async (stream?: MediaStream, _timeslice: number = 1000): Promise<void> => {
  ...
  const downloadLocalRecording = useCallback(async (filename?: string): Promise<boolean> => {
    ...
    try {
      ...
      const timestamp = (
        recordingStartTimeRef.current ? new Date(recordingStartTimeRef.current) : new Date()
      )
        .toISOString()
        .replace(/[:.]/g, '-');
      ...
      if (displayMessageRef.current) {
        displayMessageRef.current(
          `Recording downloaded: ${downloadFilename}.zip (${sizeMB} MB)`,
          'success',
        );
      }
      return true;
