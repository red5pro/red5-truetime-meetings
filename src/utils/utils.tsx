...
export function updateMetaData(metaData: string | null | undefined, key: string, value: any): string {
...
export function calculateConnectionQualityScore(
    ...
    let totalOutboundPacketsLost = outboundVideoPacketsLost + outboundAudioPacketsLost;
    ...
    if (stats.connectionType === 'subscriber') {
        ...
        metrics.packetLossPercent = (totalPacketsLost / (totalPacketsReceived + totalPacketsLost)) * 100;
    } else {
        totalPacketsSent = Math.max(totalPacketsSent, 1); // Avoid division by zero
        metrics.packetLossPercent = totalPacketsSent > 0 ? (totalOutboundPacketsLost / totalPacketsSent) * 100 : 0;
    }
    ...
    const framesPerSecond = safeGet(stats, 'inboundVideo.framesPerSecond') ||
    ...
    if (previousStats && timeDelta > 0) {
        const currentBytes = stats.connectionType === 'subscriber'
            ? safeGet(stats, 'inboundVideo.bytesReceived', 0)
            : safeGet(stats, 'outboundVideo.bytesSent', 0);

        const previousBytes = stats.connectionType === 'subscriber'
    ...
    qualityScore = (
        packetLossScore * 0.40 +    // 40% weight
        rttScore * 0.25 +           // 25% weight
        jitterScore * 0.15 +        // 15% weight
        frameRateScore * 0.10 +     // 10% weight
    ...
    metrics.resolution = stats.inboundVideo ?
        ...
        (stats.outboundVideo ? `${stats.outboundVideo.frameWidth}×${stats.outboundVideo.frameHeight}` : 'Unknown');

    metrics.codec = stats.codecs && stats.codecs.length > 0 ?
        stats.codecs.find(c => c.mimeType.includes('video'))?.mimeType || 'Unknown' : 'Unknown';
