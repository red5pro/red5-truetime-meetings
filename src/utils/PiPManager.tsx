...
class PiPManager {
    ...
    // Open PiP window
    async openWindow({ width = 800, height = 600, title = 'Video Conference' }: PiPWindowOptions = {}): Promise<Window> {
        if (!this.isSupported()) {
        ...
        <Box className="pip-participant-tile">
            ...
            {participant?.videoEnabled && mediaStream ? (
                ...
                <Box className="pip-audio-only">
                    ...
                    <Box className="pip-audio-only-name">
...
const PiPGridContent: React.FC<PiPGridContentProps> = ({
    ...
    return (
        <Box className="pip-container">
            ...
            <Box
                className="pip-participants-grid"
...
export const usePictureInPicture = (): UsePictureInPictureReturn => {
    ...
    const openPiP = useCallback(async ({
        ...
        if (!isSupported) {
            const errorMsg = 'Picture-in-Picture is not supported in this browser. Please use Chrome 111+ or Edge 111+';
            setError(errorMsg);
    ...
    // Toggle PiP
    const togglePiP = useCallback((options: PiPOpenOptions): Promise<boolean> => {
        if (isOpen) {
            closePiP();
...
export const PiPButton: React.FC<PiPButtonProps> = ({
    ...
    return (
        <Tooltip title={error || (isOpen ? 'Close Picture-in-Picture' : `Open Picture-in-Picture (${participants.length} participants)`)}>
            <IconButton
                onClick={handleClick}
                disabled={disabled}
                size={size}
